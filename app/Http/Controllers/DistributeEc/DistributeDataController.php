        ...
        if ($res_data['code'] == '200'){
            ...
            if (!empty($res_data['data']['successIdList'])){
                foreach ($res_data['data']['successIdList'] as $success_data){ //{"index": 0,"crmId": 4262563847}
                    $index_key = $success_data['index'];
                    DB::table('res_data')->where('id', $relate_customer_resData[$index_key]['res_data_id'])
                        ->update(['crmId' => $success_data['crmId'] //数据库设计为字符串即可
                            ,'ec_userId' => $relate_customer_resData[$index_key]['ec_userId']
                            ,'failureCause' => ''
                            ,'synchronize_para' => $list[$index_key] //相对应的用户
                            ,'synchronize_results'=>1
                        ]);
                    $success_EcUser_list[] = $relate_customer_resData[$index_key]['ec_userId'];
                    //操作完毕后，进行调用日志方法
                    $distribution_log_data = ['ec_userId' => $relate_customer_resData[$index_key]['ec_userId']
                        ,'failureCause' => ''
                        ,'synchronize_para' => $list[$index_key] //相对应的用户
                        ,'synchronize_results'=>1];
            ...
            if (!empty($res_data['data']['failureList'])){
                foreach ($res_data['data']['failureList'] as $failure_data){ //{"failureCause": "该客户被多人频繁操作，请稍后重试。", "index": 1}
                    $index_key2 = $failure_data['index'];
                    DB::table('res_data')->where('id', $relate_customer_resData[$index_key2]['res_data_id'])
                        ->update(['crmId' => '' //数据库设计为字符串即可
                            ,'ec_userId' => $relate_customer_resData[$index_key2]['ec_userId']
                            ,'failureCause' => $failure_data['failureCause']
                            ,'synchronize_para' => $list[$index_key2] //相对应的用户
                            ,'synchronize_results'=>0
                        ]);
                    //操作完毕后，进行调用日志方法
                    $distribution_log_data = ['ec_userId' => $relate_customer_resData[$index_key2]['ec_userId']
                        ,'failureCause' => $failure_data['failureCause']
                        ,'synchronize_para' => $list[$index_key2] //相对应的用户
                        ,'synchronize_results'=>0];
                    $this->record_distribution_log($distribution_log_data,'1');
