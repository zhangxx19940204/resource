...
class ManageVisitController extends Controller
{
    ...
    //管理来访进款表
    public function manage_visit(Request $request){
        ...
        return view('dingTalk.managevisit',['filter_user_list'=>$user_list_data,'project_list'=>$project_list,'corp_id'=>$dingTalk_arr['corp_id']]);
    }
    //获取列表
    public function get_manage_visit_list(Request $request){
        ...
        if (array_key_exists('filter_blong',$para) && !empty($para['filter_blong'])){
            $where_arr[] = ['dingding_visit.blong','=',$para['filter_blong']];
        }
        if (array_key_exists('filter_date',$para) && !empty($para['filter_date'])){
            $where_arr[] = ['dingding_visit.visit_date','=',$para['filter_date']];
        }
        if (array_key_exists('filter_phone',$para) && !empty($para['filter_phone'])){
            $where_arr[] = ['dingding_visit.phone','=',$para['filter_phone']];
        }
        $member_arr = [];
